...
def init_graph_query(question_node_class, question_node_friendly_name=None):
    graph_query = {'nodes': [], 'edges': []}
    add_question_node(graph_query, question_node_class, question_node_friendly_name)
    return graph_query
...
def get_literals(graph_query: dict):
    res = set()
    for node in graph_query['nodes']:
...
def is_question_node(graph_query, nid):
    return graph_query['nodes'][nid]['question_node'] == 1
