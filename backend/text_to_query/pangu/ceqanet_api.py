...
def convert_ppod_kg_lisp_to_kg_api_lisp(lisp: str):
    for token in lisp.split():
        ...
        if inv is False:  # could be entity or relation
            label = ceqanet_pangu_for_sparql.entity_to_label.get(token, None)
            if label is not None:
                uuid = ceqanet_pangu_for_kg_api.label_to_entity.get(label, None)
                if uuid is not None:
                    lisp = lisp.replace(token, f"[{uuid[0]}]")

        label = ceqanet_pangu_for_sparql.predicate_to_label.get(token, None)
        if label is not None:
            label = '_'.join(label.split(' '))
            uuid = ceqanet_pangu_for_kg_api.label_to_predicate.get(label, None)
            if uuid is not None:
                lisp = lisp.replace(token, f"[{uuid[0]}]")
...
class PanguForCEQAnet:
    def __init__(self, proj_root: str = None, api_key: str = '', llm_name: str = 'gpt-4o', retriever: str = 'sentence-transformers/gtr-t5-base', use_kg_api=False):
        ...
        if use_kg_api:
            ...
            # load data
            self.entity_to_label = json.load(open(os.path.join(proj_root, 'ceqanet_data/entity_to_label.json'), 'r'))
            self.label_to_entity = json.load(open(os.path.join(proj_root, "ceqanet_data/label_to_entity.json"), 'r'))
            self.predicate_to_label = json.load(open(os.path.join(proj_root, "ceqanet_data/predicate_to_label.json"), 'r'))
            self.label_to_predicate = json.load(open(os.path.join(proj_root, "ceqanet_data/label_to_predicate.json"), 'r'))
            self.kb_relations = json.load(open(os.path.join(proj_root, "ceqanet_data/predicates.json"), 'r'))
            self.kb_classes = json.load(open(os.path.join(proj_root, "ceqanet_data/types.json"), 'r'))
            self.literals = json.load(open(os.path.join(proj_root, "ceqanet_data/literals.json"), 'r'))
            self.property_to_entity = json.load(open(os.path.join(proj_root, "ceqanet_data/predicate_entity.json"), 'r'))
            self.property_to_literal = json.load(open(os.path.join(proj_root, "ceqanet_data/predicate_literal.json"), 'r'))
        self.prefixes = json.load(open(os.path.join(proj_root, "ceqanet_data/prefix.json"), 'r'))
    ...
    def text_to_query(self, question: str, top_k: int = 10, max_steps: int = 3, verbose: bool = False, api_key: str = None):
        ...
        if not self.use_kg_api:
            for plan in final_plans[:30]:
                ...
                rows = execute_query(sparql, endpoint='localhost:3003/sparql')
                if len(rows) > 0:
                    num_valid_query += 1
...
ceqanet_pangu_for_sparql = PanguForCEQAnet(llm_name='gpt-4o', use_kg_api=False)
ceqanet_pangu_for_kg_api = PanguForCEQAnet(llm_name='gpt-4o', use_kg_api=True)
