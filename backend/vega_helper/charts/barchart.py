...
class BarChart(Chart):
    ...
    def plot(self):
        ...
        """
        print('cd plot function')
        if self.promote_to_candidate():
            print('into promote')
            vega_script  = self.draw()
            print(vega_script)
        else:
            print('fail to promote')
            pass
    ...
    def genTemplate(self, data, x, y, color=None):
        ...
        tem['data']['values'] = data
        tem['encoding']['x']['field'] = x
        tem['encoding']['y']['field'] = y
        tem['color']['field'] = color 
        return tem
    ...
    def draw(self):
        ...
        if len(categorical_column) > 0:
            if orientation is not None:
                return self.genTemplate(self, self.dataframe, numerical_label, item_column[0],categorical_column[0])
                # fig = px.bar(self.dataframe, x=numerical_label, y=item_column[0], color=categorical_column[0])
                # fig.show()
            else:
                return self.genTemplate(self, self.dataframe, item_column[0], numerical_label, categorical_column[0])
            ...
            if orientation is not None:
                ...
                return self.genTemplate(self, data, numerical_label, item_column[0])
                # fig = px.bar(data, x=numerical_label, y=item_column[0])
                # fig.show()
            else:
                data = self.dataframe.sort_values(by=[numerical_label], ascending=False)
                return self.genTemplate(self, data, item_column[0], numerical_label)
