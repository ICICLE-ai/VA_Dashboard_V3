...
<template>
  <div>
    ...
    <v-card class="node-card" :width=cardWidth :height=cardHeight variant="outlined" :title=type :style="{
      ...
      <div>
        <v-text-field v-model="endpoint" label="KG Endpoint" required></v-text-field>
        <v-btn @click="sparql_query()">Query
          <v-progress-circular v-if="loading_sparql_query_result" :model-value="progress" :rotate="360" :size="30"
            :width="5" color="teal">
          </v-progress-circular>
        </v-btn>
      </div>
