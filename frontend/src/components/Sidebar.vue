  ...
  <aside class="sidebar-container">
    <div class="scrollable-content">
      ...
      <div>
        ...
        <v-row>
          <div class="drag-node" :draggable="true"
            @dragstart="onDragStart($event, { type: 'tableViewer', title: 'Table Viewer' })">
            <font-awesome-icon :icon="['fas', 'table']" class="drag-icon" /> Table Viewer
          </div>
        </v-row>
        <v-row>
          <div class="drag-node" :draggable="true"
            @dragstart="onDragStart($event, { type: 'graphViewer', title: 'Graph Viewer' })">
            <font-awesome-icon :icon="['fas', 'diagram-project']" class="drag-icon" />Graph Viewer
          </div>
        </v-row>
        <v-row>
          <div class="drag-node" :draggable="true"
            @dragstart="onDragStart($event, { type: 'sparqlEditor', title: 'SPARQL Editor' })">
            <font-awesome-icon :icon="['fas', 'pen-to-square']" class="drag-icon" />SPARQL Editor
          </div>
        </v-row>
        <v-row>
          <div class="drag-node" :draggable="true"
            @dragstart="onDragStart($event, { type: 'sparqlExecutor', title: 'SPARQL Executor' })">
            <font-awesome-icon :icon="['fas', 'pen-to-square']" class="drag-icon" />SPARQL Executor
          </div>
        </v-row>
        <!-- <v-row>
        ...
        <v-row>
          <div class="drag-node" :draggable="true"
            @dragstart="onDragStart($event, { type: 'vegaLite', title: 'VegaLite' })">
            <font-awesome-icon :icon="['fas', 'circle-dot']" class="drag-icon" />VegaLite
          </div>
        </v-row>
        <!-- <v-row>
        ...
        <v-row>
          <v-form ref="form">
            ...
            <v-card class="mb-4">
              ...
              <v-card-text>
                <v-text-field v-model="wakeWord" label="Wakeword" class="mb-4"></v-text-field>
            ...
            <v-card class="mb-4 mt-4">
              ...
              <v-text-field v-model="question" label="Question" style="width: 100%;" required></v-text-field>
            </v-card>
          ...
          ></v-text-field> -->
            ...
            <v-btn @click="sendMessage">
              Explore
              <v-progress-circular v-if="loading" :model-value="progress" :rotate="360" :size="30" :width="5"
                color="teal">
              </v-progress-circular>
            </v-btn>
            <v-btn @click="text2query()">Text2Query
              <v-progress-circular v-if="loading_text2query" :model-value="progress" :rotate="360" :size="30" :width="5"
                color="teal">
              </v-progress-circular>
            </v-btn>
